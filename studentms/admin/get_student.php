...
if(!empty($_POST["classid"])) 
{
 ...
 $stmt = $dbh->prepare("SELECT StudentName,ID FROM tblstudent WHERE ClassId= :id order by StudentName");
...
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT ID,ClassId FROM tblresult WHERE StudentId=:id1 and ClassId=:id ");
